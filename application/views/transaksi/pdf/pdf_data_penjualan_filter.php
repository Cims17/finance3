...
private $widths;
  ...
  function NbLines($w,$txt)
  {
	...
	// Page Content
	function Content($penjualan, $jumlah_penjualan,$tgl){
		$border = 0;
	//   $this->AddPage();
	  ...
	  $this->Cell(0, 6, "Laporan Data Penjualan",0,1,'C',1);
	  ...
	  $this->SetX($left += 30); $this->Cell(50, $h, 'Pelanggan', 1, 0, 'C',true);
	  ...
	  foreach ($penjualan as $pjl) {
		  $this->Row(
			  ...
			  $pjl['keterangan'],
			  $pjl['tanggal'],
			  "Rp.". number_format($pjl['total'])
		  ));
	  }
	  $this->SetFont('Arial','B',10);
	  $this->Cell(161,$h,'Total Penjualan',1,0,'C',true);
      foreach ($jumlah_penjualan as $jml_pjl) {
          $this->Cell(30, $h, "Rp.". number_format($jml_pjl['totalPenjualan']), 1, 0, 'C', true);
      }
	}
...
$this->myfpdf->AddPage();
$this->myfpdf->SetTitle('LAPORAN DATA PENJUALAN TOKO ANITA',true);
$this->myfpdf->Content($penjualan, $jumlah_penjualan,$tgl);
$this->myfpdf->Output('laporan-data-penjualan-toko-anita ['.date('d-M-Y').'].pdf','I');
?>
